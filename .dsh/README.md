...
将 `.dsh` 目录复制到 `$DSH_HOME/.agent-presets/sports-pipeline/`：

```powershell
# Windows PowerShell 7
$dst = Join-Path $env:DSH_HOME ".agent-presets\sports-pipeline"
Copy-Item -Recurse -Force ".\dsh" $dst
```

重启 DeepSeek Harness 后，在会话预设列表选择 `运动内容管线`。

### 方式二：仅安装技能（skill）

将 SKILL.md 复制到 `$DSH_HOME/skills/dsh-sports-pipeline/`：

```powershell
$dst = Join-Path $env:DSH_HOME "skills\dsh-sports-pipeline"
New-Item -ItemType Directory -Force -Path $dst | Out-Null
Copy-Item ".\skills\dsh-sports-pipeline\SKILL.md" $dst -Force
```
